...
const axios = require('../extend/axios')

class Weather {
    constructor() {
        this.baseUrl = 'http://weather1.sina.cn'
    }
    ...
    async query(code) {
        ...
        if (code) {
            _url = this.baseUrl + `/?code=${code}&vt=4&cf=c`
        }
        let html = await axios.http(_url)
        let $ = cheerio.load(html)
        let $con = $('.inx_w_cont').children('.inx_w_cont_dl')
        let result = {}

        result.cityName = $con.find('.inx_w_city_c').text()

        result.weather = {}
        let _mare = $con.find('.inx_w_r_mare').text().trim().split('\n')
        result.weather.value = _mare.length ? _mare[0] : ''
        result.weather.img = $con.find('.inx_w_pic').children('img').attr('src')
        ...
        // 空气质量
        result.aqi = $con.find('.inx_link_tips').find('b').text()

        // 温度
        result.tmp = $con.find('.inx_w_r_num').children('strong').text()
        ...
        result.update = $con.find('.inx_w_r_time').text().replace('更新', '').trim().split(' ')[1]
